    ...
    private static class Worker implements Runnable {
        ...
        private void close() {
            ...
            if (activeIdsIndexes.containsKey(id)) {
                ...
                int lastId = activeIds.get(activeIds.size() - 1); // Replace getLast()
                activeIds.set(index, lastId);
                activeIdsIndexes.put(lastId, index);
                activeIds.remove(activeIds.size() - 1); // Replace removeLast()
                activeIdsIndexes.remove(id);
            }
            lock.unlock();
        }

        // Forced transfer to test deadlock scenario
        private void forcedTransfer() { /*---------------------------------------------------------------*/
            int from = 0; // Choose a fixed account, e.g., ID 0
            int to = 1; // Choose a second fixed account, e.g., ID 1
            bank.transfer(from, to, 1);
            bank.transfer(to, from, 1); // Transfer in the opposite direction
        }
        ...
        public void run() {
            for (int i = 0; i < OPS_PER_WORKER; i++) {
                ...
                // Introduce a forced deadlock scenario occasionally
                if (i % (OPS_PER_WORKER / 10) == 0) { // every 10% of OPS_PER_WORKER
                    forcedTransfer(); /*---------------------------------------------------------------*/
                }
            }
        }
